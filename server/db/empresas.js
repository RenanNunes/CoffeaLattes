...
const schema = Joi.object().keys({
    ...
    */
    localidades: Joi.array().items(
        Joi.string().max(100).allow('').error(new Error('Cada localidade da empresa pode conter até 100 caracteres'))),
    areaDeAtuacao: Joi.string().max(75).allow('').error(new Error('Área de atuação pode conter até 75 caracteres')),
    descricao: Joi.string().max(500).allow('').error(new Error('A descrição da empresa pode conter menos de 500 caracteres')),
    site: Joi.string().regex(new RegExp(/^(http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)?[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$/)).allow('').error(new Error('Insira um formato válido para o site')),
